...
void PC2Beolink::process_beo4_keycode(uint8_t type, uint8_t keycode) {
    ...
    if (this->pc2->keystroke_callback != nullptr) {
...
void PC2::init() {
    ...
    //this->device->write({ 0x80, 0x01, 0x00 }); // The Beomedia 1 sends 80 29 here; 80 01 seems to work, too; as does 0x00 and 0xFF
    ...
    //this->mixer->speaker_power(true);
    //this->device->write({ 0x29 }); // Requests software version?
    // sends an ACK-ish message (60 05 49 02 36 01 04 61); then another (60 02 0A 01 61).
    //set_address_filter();
    //this->device->write({0xe0, 0xC1, 0xC0, 0x01, 0x14, 0x00, 0x00, 0x00, 0x04, 0x02, 0x04, 0x02, 0x01});
    /*this->device->write({ 0xe0, 0x83, 0xc2, 0x01, 0x14, 0x00, 0x47, 0x00, \
    ...
    //this->device->write({ 0x26 }); // get status info (PC2 will send a 06 00/01 to say if HP are in)
    //// Check if a video master is present?
    //this->device->write({ 0xe0,0xc0,0xc1,0x01,0x0b,0x00,0x00,0x00,0x04,0x03,0x04,0x01,0x02,0x00,0x9b,0x00 });
    //expect_ack();
    //yield();
    //// Some sort of status message or alive message, addressed at video master
    //this->device->write({ 0xe0,0xc0,0x01,0x01,0x0b,0x00,0x00,0x00,0x04,0x01,0x17,0x01,0xea,0x00 });
    //yield();

    //        send_beo4_code(0xc0, 0x91);
    //this->mixer->speaker_power(false);
    // look for video masters?
    //this->device->write({0xe0, 0xc0, 0x01, 0x01, 0x0b, 0x00, 0x00, 0x00, 0x04, 0x03, 0x04, 0x0a, 0x01, 0x00, 0xe3, 0x00});
    // look for audio masters?
        //this->device->write({0xe0, 0xc1, 0x01, 0x01, 0x0b, 0x00, 0x00, 0x00, 0x04, 0x03, 0x04, 0x0a, 0x01, 0x00, 0xe4, 0x00});
}
